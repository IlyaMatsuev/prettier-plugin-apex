...
# In another terminal
npx jest -c jest.config.standalone.js
...
- You can make changes and run `npx jest -u -c jest.config.standalone.js` to update the snapshots. Then run `git diff` to take a look at what changed. Always update the snapshots when opening a PR.
