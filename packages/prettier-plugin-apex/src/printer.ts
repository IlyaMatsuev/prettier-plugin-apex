...
/**
 * Applies the fork's modifier-related options (`apexFormatAnnotations`,
 * `apexExplicitAccessModifier`, `apexSortModifiers`) to the printed modifier
 * docs of a declaration.
 */
function normalizeModifiers(
  ...
  let nonAnnotationModifiers: Doc[] = modifiers
...
/**
 * Collapses a getter/setter block onto one line when it only holds a single
 * statement (`apexExpandOneLineProperties: false`).
 */
...
/**
 * Prints the body of an if/else/for/while, wrapping a non-block statement in
 * curly braces when `apexForceCurly` is enabled.
 */
...
/**
 * Pushes the closing `}` of a declaration/block body, keeping it on the same
 * line as the opening brace for empty bodies when `apexEmptyBlockBracketLine`
 * is enabled.
 */
function pushClosingBrace(
  parts: Doc[],
  isEmpty: boolean,
  options: ApexParserOptions,
): void {
  if (!isEmpty || !options.apexEmptyBlockBracketLine) {
    parts.push(hardline);
  }
  parts.push("}");
}
...
function handleTriggerDeclarationUnit(
  ...
  pushClosingBrace(
    parts,
    danglingCommentDocs.length === 0 && memberDocs.length === 0,
    options,
  );
  return parts;
}
...
function handleAnnotation(
  ...
  parts.push("@");
  let annotationName: Doc = path.call(print, "name", "value");
...
function handleAnnotationKeyValue(
  ...
  const parts: Doc[] = [];
  let argName: Doc = path.call(print, "key", "value");
  let separator = "=";
  if (options.apexFormatAnnotations) {
...
function handleVariableDeclarations(
  ...
  const parts: Doc[] = [];
  // Modifiers
  if (getParentNode(path)?.["@class"] === APEX_TYPES.FIELD_MEMBER) {
