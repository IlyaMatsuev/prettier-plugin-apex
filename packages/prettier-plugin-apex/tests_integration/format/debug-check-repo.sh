...
pnpm --filter=@ilyamatsuev/prettier-plugin-apex run debug-check:built-in "$location/$name/**/{classes,triggers}/**/*.{cls,trigger}" --no-config --ignore-path=./tests_integration/format/ignore-list.txt
rm -rf "$location/$name"
